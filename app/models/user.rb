...
class User < ActiveRecord::Base
  ...
  scope :with_two_factor,    -> { joins("LEFT OUTER JOIN u2f_registrations AS u2f ON u2f.user_id = users.id").
                                    where("u2f.id IS NOT NULL OR otp_required_for_login = true").group("users.id") }
  scope :without_two_factor, -> { joins("LEFT OUTER JOIN u2f_registrations AS u2f ON u2f.user_id = users.id").
